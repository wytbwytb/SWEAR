...
@ROTATED_HEADS.register_module()
class RefineOrientedRPNHead(RotatedRPNHead):
    """Oriented RPN head for Oriented R-CNN."""
    def __init__(self,
                 in_channels,
                 bbox_coder_h,
                 ...
                 version='oc',
                 **kwargs):
        super(RefineOrientedRPNHead, self).__init__(
            in_channels, init_cfg, version, **kwargs)
    ...
    def _init_layers(self):
        ...
        self.rpn_cls = nn.Conv2d(self.feat_channels,
                                 self.num_anchors * self.cls_out_channels, 1)
        self.rpn_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 6, 1)
        self.rpn_reg_h = nn.Conv2d(self.feat_channels, self.num_anchors * 4, 1)
    ...
    def forward_single(self, x):
        ...
        rpn_bbox_pred_h = self.rpn_reg_h(x)
        assert torch.isnan(rpn_bbox_pred).sum() == 0, print(rpn_bbox_pred)
        return rpn_cls_score, rpn_bbox_pred, rpn_bbox_pred_h
    ...
    def _get_targets_single(self,
        ...
        bbox_targets_h = anchors.new_zeros((anchors.size(0), 4))
        bbox_weights_h = anchors.new_zeros((anchors.size(0), 4))
        labels = anchors.new_full((num_valid_anchors, ),
                                  self.num_classes,
                                  dtype=torch.long)
        label_weights = anchors.new_zeros(num_valid_anchors, dtype=torch.float)
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(
                    sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes)
                pos_gt_bboxes_h = obb2xyxy(sampling_result.pos_gt_bboxes, self.version)
                pos_bbox_targets_h = self.bbox_coder_h.encode(
                    sampling_result.pos_bboxes, pos_gt_bboxes_h
                )
            else:
                pos_bbox_targets = sampling_result.pos_gt_bboxes
            bbox_targets[pos_inds, :] = pos_bbox_targets
            bbox_weights[pos_inds, :] = 1.0
            bbox_targets_h[pos_inds, :] = pos_bbox_targets_h
            bbox_weights_h[pos_inds, :] = 1.0
            if gt_labels is None:
                # Only rpn gives gt_labels as None
                # Foreground is the first class since v2.5.0
                labels[pos_inds] = 0
        ...
        if unmap_outputs:
            ...
            bbox_targets = unmap(bbox_targets, num_total_anchors, inside_flags)
            bbox_weights = unmap(bbox_weights, num_total_anchors, inside_flags)
            bbox_targets_h = unmap(bbox_targets_h, num_total_anchors, inside_flags)
            bbox_weights_h = unmap(bbox_weights_h, num_total_anchors, inside_flags)

        return (labels, label_weights, bbox_targets, bbox_weights, bbox_targets_h, bbox_weights_h, pos_inds,
                neg_inds, sampling_result)
    ...
    def get_targets(self,
        ...
        (all_labels, all_label_weights, all_bbox_targets, all_bbox_weights, all_bbox_targets_h, all_bbox_weights_h,
        ...
        bbox_weights_list = images_to_levels(all_bbox_weights,
                                             num_level_anchors)
        bbox_targets_h_list = images_to_levels(all_bbox_targets_h,
                                             num_level_anchors)
        bbox_weights_h_list = images_to_levels(all_bbox_weights_h,
                                             num_level_anchors)
        res = (labels_list, label_weights_list, bbox_targets_list,
               bbox_weights_list, bbox_targets_h_list, bbox_weights_h_list, num_total_pos, num_total_neg)
    ...
    def loss_single(self, cls_score, bbox_pred, bbox_pred_h, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, bbox_targets_h, bbox_weights_h, num_total_samples):
        """Compute loss of a single scale level.
        ...
        bbox_targets = bbox_targets.reshape(-1, 6)
        bbox_weights = bbox_weights.reshape(-1, 6)
        bbox_targets_h = bbox_targets_h.reshape(-1, 4)
        bbox_weights_h = bbox_weights_h.reshape(-1, 4)
        bbox_pred = bbox_pred.permute(0, 2, 3, 1).reshape(-1, 6)
        bbox_pred_h = bbox_pred_h.permute(0, 2, 3, 1).reshape(-1, 4)
        anchors = anchors.reshape(-1, 4)
        # pos_ids = bbox_weights[1,:]
        proposals = self.bbox_coder.decode(
            anchors, bbox_pred)
        gts_h = self.bbox_coder_h.decode(
            anchors, bbox_targets_h)
        proposals_hh = self.bbox_coder_h.decode(
            anchors, bbox_pred_h)
        
        proposals_h = obb2xyxy(proposals, self.version)
        # gts_h = obb2xyxy(gts, self.version)

        # bbox_targets_h = self.bbox_coder_h.encode(gts_h, anchors)
        bbox_pred_rh = self.bbox_coder_h.encode(anchors, proposals_h)
        # bbox_h_weights = bbox_weights[:,:4]
        ...
        loss_bbox = self.loss_bbox(
            ...
            bbox_weights,
            avg_factor=num_total_samples)
        loss_bbox_h = self.loss_bbox(
            bbox_pred_h,
            bbox_targets_h,
            bbox_weights_h,
            avg_factor=num_total_samples)
        loss_bbox_rh = self.loss_bbox_h(
            proposals_h,
            gts_h,
            bbox_weights_h,
            avg_factor=num_total_samples)
        
        # loss_bbox_rh = self.loss_bbox(
        #     bbox_pred_h,
        #     bbox_pred_rh,
        #     bbox_weights_h,
        #     avg_factor=num_total_samples)
        # print(loss_bbox_h)
        # if loss_bbox_h > 1 or torch.isinf(loss_bbox_h).sum() > 0 or torch.isnan(loss_bbox_h).sum() > 0:
        #     assert torch.isnan(loss_bbox_h).sum() == 0, print(loss_bbox_h)
        # assert torch.isnan(loss_bbox_h).sum() == 0, print(loss_bbox_h)
        return loss_cls, loss_bbox, loss_bbox_h, loss_bbox_rh
    ...
    def loss(self,
        ...
        if cls_reg_targets is None:
            return None
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list, bbox_targets_h_list, bbox_weights_h_list,
        ...
        losses_cls, losses_bbox, losses_bbox_h, losses_bbox_rh = multi_apply(
            self.loss_single,
            cls_scores,
            bbox_preds,
            bbox_preds_h,
        ...
        self.iter += 1
        # if self.iter > 0:
        #     return dict(loss_rpn_cls=losses_cls, loss_rpn_bbox=losses_bbox, loss_rpn_bbox_h=losses_bbox_h)
        # else:
        #     return dict(loss_rpn_cls=losses_cls, loss_rpn_bbox=losses_bbox)
        return dict(loss_rpn_cls=losses_cls, loss_rpn_bbox=losses_bbox)
        # return dict(loss_rpn_cls=losses_cls, loss_rpn_bbox=losses_bbox, loss_rpn_bbox_rh=losses_bbox_rh)
