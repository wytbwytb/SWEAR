...
class OursRPNHead(RotatedRPNHead):
    ...
    def _get_targets_single(self,
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                ...
                # if torch.any(torch.isnan(pos_bbox_targets)):
                #     print(torch.any(torch.isnan(pos_bbox_targets)))
            else:
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        #     print(torch.any(torch.isnan(bbox_targets)))
        # classification loss
        ...
        # if torch.any(torch.isnan(loss_bbox)):
        #     print(torch.any(torch.isnan(bbox_pred)),torch.any(torch.isnan(bbox_targets)))
        # print('loss_rpn_cls:', loss_cls, 'loss_rpn_bbox', loss_bbox)
