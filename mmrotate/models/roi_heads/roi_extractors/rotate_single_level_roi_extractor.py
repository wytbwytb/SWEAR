...
@ROTATED_ROI_EXTRACTORS.register_module()
class RefineRotatedSingleRoIExtractor(RotatedSingleRoIExtractor):
    """Extract RoI features from a single level feature map.
    ...
    @force_fp32(apply_to=('feats', ), out_fp16=True)
    def forward(self, feats, rois, rois_gt=None, roi_scale_factor=None):
        """Forward function.
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            roi_feats = feats[0].new_zeros(
                rois.size(0), self.out_channels, *out_size)
            if rois_gt is not None:
                roi_gt_feats = feats[0].new_zeros(
                rois_gt.size(0), self.out_channels, *out_size)
        # TODO: remove this when parrots supports
        if torch.__version__ == 'parrots':
            roi_feats.requires_grad = True
        ...
        for i in range(num_levels):
            ...
            if inds.numel() > 0:
                ...
                if rois_gt is not None:
                    rois_gt_ = rois_gt[inds]
                    roi_gt_feats_t = self.roi_layers[i](feats[i], rois_gt_)
                    roi_gt_feats[inds] = roi_gt_feats_t
            else:
                # Sometimes some pyramid levels will not be used for RoI
                # feature extraction and this will cause an incomplete
                # computation graph in one GPU, which is different from those
