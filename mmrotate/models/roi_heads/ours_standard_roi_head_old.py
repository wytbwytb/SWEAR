...
class OursStandardRoIHead(RotatedStandardRoIHead):
    ...
    def __init__(self,
        ...
        self.prototype_features = [torch.rand((bbox_head['fc_out_channels'])).cuda() for _ in range(bbox_head['num_classes'])]
        self.extractor = nn.Linear(in_features=bbox_head['fc_out_channels'], out_features=bbox_head['fc_out_channels'])
        self.gate = False
        self.iters = 0
        self.warmup = 0 #3000
        self.warmup_2 = 0 #3400
        # self.warmup = 0
        # self.warmup_2 = 50
    ...
    #         feats_sim_all = feats_sim_all_temp
            ...
            x_gt, y_gt = self.label2onehot(pos_angles)
            pos_feats_sim = self.simulator(pos_feats_flatten, x_gt, y_gt)
            
            self.check_level3_positive(pos_feats_flatten, x_gt, y_gt, pos_feats_sim, pos_gts)
            # self.draw_tsne()
            # print(feats_sim)
        ...
        if self.iters > self.warmup_2:
            ...
            loss_sim2 = self.simulator.loss_simulate(pos_feats_sim, pos_gts, prototype_features)
            loss_bbox['loss_simulate2'] = loss_sim2 
            ...
            loss_bbox['loss_angle_aftersim'] = loss_angle_aftersim * 0.5
