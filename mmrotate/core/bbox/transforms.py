...
def poly2obb_cc(poly):
    ...
    Returns:
        obbs (torch.Tensor): [x_ctr, y_ctr, r, angle1, angle2]
    """
    x0, y0, x1, y1, x2, y2, x3, y3 = torch.split(poly, [1, 1, 1, 1, 1, 1, 1, 1], dim=-1)

    x_c = (x0 + x2) / 2.0
    y_c = (y0 + y2) / 2.0

    pt_x = torch.cat([x0, x1, x2, x3], dim=-1)
    pt_y = torch.cat([y0, y1, y2, y3], dim=-1)
    
    sorted, sorted_id = torch.sort(pt_y, dim=1, descending=True)
    ymax_idx= sorted_id[:,:2]
    y_select = torch.gather(pt_y, dim=-1, index=ymax_idx)
    x_select = torch.gather(pt_x, dim=-1, index=ymax_idx)
    _, pt1_idx = torch.max(x_select, dim=-1,keepdim=True)
    pt2_idx = 1 - pt1_idx


    x1, y1 = torch.gather(x_select, dim=-1, index=pt1_idx), torch.gather(y_select, dim=-1, index=pt1_idx)
    x2, y2 = torch.gather(x_select, dim=-1, index=pt2_idx), torch.gather(y_select, dim=-1, index=pt2_idx)
    ...
    # y2 = xc + rsin(a1+a2)
    # --->
    # a1 = arctan((y1 - yc) / (x1 - xc)) 
    # r = (x1 - xc) / cosa1
    # a2 = arccos((x2 - xc) / r) - a1 

      
    a1 = torch.atan((y1 - y_c) / (x1 - x_c))
    a1 = torch.where(a1 >= 0, a1, a1 + np.pi)
    # r = (x1 - x_c) / torch.cos(a1)
    r1 = (x1 - x_c) / torch.cos(a1)
    r2 = (y1 - y_c) / torch.sin(a1)
    r1[torch.isnan(r1)] = -1
    r2[torch.isnan(r2)] = -1
    r = torch.max(r1, r2)
    # print(torch.min(r))
    # a2 = torch.acos((x2 - x_c) / r) - a1
    a2 = torch.atan((y2 - y_c) / (x2 - x_c)) - a1
    a2 = torch.where(a2 >= 0, a2, a2 + np.pi)
    # if torch.any(torch.isnan(a2)):
    #     print(torch.any(torch.isnan(x2)),torch.any(torch.isnan(x_c)),torch.any(torch.isnan(r)),torch.any(torch.isnan(a1)))
    cc = torch.cat([x_c, y_c, r, a1, a2], dim=1)
    if torch.any(torch.isnan(cc)):
        print(torch.any(torch.isnan(r)))
    
    return torch.cat([x_c, y_c, r, a1, a2], dim=1)
...
def obb2poly_cc(rbboxes):
    ...
    a1 = rbboxes[:, 3]
    a2 = rbboxes[:, 4]
    # p_1=(x+rcos⁡(a_1),y+rsin⁡(a_1)), 
    # p_2=(x+rcos⁡(a_1+a_2),y+rsin⁡(a_1+a_2 )
    # p_3=2*(x,y)-p1
    # p_4=2*(x,y)-p2
    pt1x = x + r * torch.cos(a1)
    pt1y = y + r * torch.sin(a1)
    pt2x = x + r * torch.cos(a1 + a2)
    pt2y = y + r * torch.sin(a1 + a2)
    pt3x = 2 * x - pt1x
    pt3y = 2 * y - pt1y
    pt4x = 2 * x - pt2x
    pt4y = 2 * y - pt2y

    return torch.stack([pt1x, pt1y, pt2x, pt2y, pt3x, pt3y, pt4x, pt4y], dim=-1)
...
def xyxy2poly(bboxes):
    """Convert xyxy bboxes to polygons.
    ...
    Returns:
        polys (torch.Tensor): [x0,y0,x1,y1,x2,y2,x3,y3]
    """
    xmin, ymin, xmax, ymax = torch.split(bboxes, [1, 1, 1, 1], dim=-1)
    pt1 = torch.cat([xmin, ymin], 1)
    pt2 = torch.cat([xmax, ymin], 1)
    pt3 = torch.cat([xmax, ymax], 1)
    pt4 = torch.cat([xmin, ymax], 1)
    return torch.cat([pt1, pt2, pt3, pt4], 1)
