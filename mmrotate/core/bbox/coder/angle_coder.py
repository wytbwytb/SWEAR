...
class CSLCoder(BaseBBoxCoder):
    ...
    def encode(self, angle_targets):
        ...
        # c = smooth_label.scatter(1, radius_range, smooth_value)
...
class SXYCoder(BaseBBoxCoder):
    ...
    def __init__(self, window='gaussian', radius=6):
        super().__init__()
        assert window in ['gaussian', 'triangle', 'rect', 'pulse']
        self.angle_range = 90
        self.window = window
        self.radius = radius
        self.level_range = [(0, 50), (50, 70), (70, 90)]
        self.coding_len = 3
    ...
    def encode(self, angle_targets):
        ...
        # empty label
        smooth_label_angle = torch.zeros_like(angle_targets).repeat(
            1, self.angle_range)

        if self.window == 'pulse':
            radius_range = angle_targets
            smooth_value = 1.0
        elif self.window == 'rect':
            base_radius_range = torch.arange(
                -self.radius, self.radius, device=angle_targets.device)
            radius_range = base_radius_range + angle_targets
            smooth_value = 1.0
        elif self.window == 'triangle':
            base_radius_range = torch.arange(
                -self.radius, self.radius, device=angle_targets.device)
            ...
            raise NotImplementedError
        radius_range = radius_range.long()
        radius_range[radius_range < 0] = 0
        radius_range[radius_range >= self.angle_range] = 0
        if isinstance(smooth_value, torch.Tensor):
            smooth_value = smooth_value.unsqueeze(0).repeat(
                smooth_label_angle.size(0), 1)
        smooth_label_angle = smooth_label_angle.scatter(1, radius_range, smooth_value)
        smooth_label_angle[:, 0] = 0
        smooth_label = torch.zeros_like(angle_targets).repeat(
            1, self.coding_len)

        l1 = torch.sum(smooth_label_angle[:, 0:50], dim=-1, keepdim=True)
        l2 = torch.sum(smooth_label_angle[:, 50:70], dim=-1, keepdim=True)
        l3 = torch.sum(smooth_label_angle[:, 70:90], dim=-1, keepdim=True)
        smooth_label = torch.cat([l1, l2, l3], dim=-1)
        smooth_label = torch.div(smooth_label.T, torch.sum(smooth_label, dim=-1)).T
        return smooth_label
