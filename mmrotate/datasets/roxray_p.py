...
from mmrotate.core.evaluation.eval_map import *
from .builder import ROTATED_DATASETS


@ROTATED_DATASETS.register_module()
class ROXrayDataset_P(CustomDataset):
    ...
    def __init__(self,
        ...
        super(ROXrayDataset_P, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def prepare_train_img(self, idx):
        ...
        results['simulated'] = DC(to_tensor(ann_info['simulated']))
        results['angles'] = DC(to_tensor(ann_info['angles']))
        # results['flip'] = None
        
        if self.proposals is not None:
            results['proposals'] = self.proposals[idx]
        self.pre_pipeline(results)
        c = self.pipeline(results)
        return c
    ...
    def load_annotations(self, ann_folder):
        ...
        for ann_file in ann_files:
            ...
            with open(ann_file) as f:
                ...
                for si in s:
                    ...
                    try:
                        ...
                        # print(ann_file, s)
                        continue
                    simulated = bbox_info[1]
                    angles = bbox_info[2:5]
            ...
            if gt_bboxes:
                ...
                data_info['ann']['simulated'] = np.array(
                    gt_simulated, dtype=np.int64)
                data_info['ann']['angles'] = np.array(
                    gt_angles, dtype=np.float32)
            else:
                ...
                data_info['ann']['polygons'] = np.zeros((0, 8),
                                                        dtype=np.float32)
                data_info['ann']['simulated'] = np.array([], dtype=np.int64)
                data_info['ann']['angles'] = np.zeros((0, 3),
                                                        dtype=np.float32)
