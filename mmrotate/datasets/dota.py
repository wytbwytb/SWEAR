...
from mmrotate.core import eval_rbbox_map, obb2poly_np, poly2obb_np, hbb2obb
from mmrotate.core.evaluation.eval_map import eval_rbbox_map_angle, eval_map
...
class DOTADataset(CustomDataset):
            ...
            for ann_file in ann_files:
                ...
                with open(ann_file) as f:
                    ...
                    for si in s:
                        ...
                        x1, y1, x2, y2, x3, y3, x4, y4 = [float(_) for _ in bbox_info[:8]]
                        xmin = min([x1, x2, x3, x4])
                        xmax = max([x1, x2, x3, x4])
                        ymin = min([y1, y2, y3, y4])
                        ymax = max([y1, y2, y3, y4])
                        bbox = np.array([xmin, ymin, xmax, ymax], dtype=np.float32)
                        bbox_r = hbb2obb(torch.from_numpy(bbox).unsqueeze(dim=0), self.version).squeeze().numpy()
                        try:
    ...
    def evaluate(self,
        ...
        if metric == 'mAP':
            ...
            # mean_ap, _ = eval_rbbox_map_horizontal_level(
            #     results,
            #     annotations,
            #     scale_ranges=scale_ranges,
            #     iou_thr=iou_thr,
            ...
            #     nproc=nproc)
            # eval_results['mAP'] = mean_ap
            mean_ap, _ = eval_rbbox_map_angle(
                results,
                annotations,
                scale_ranges=scale_ranges,
                iou_thr=iou_thr,
                dataset=['angle1', 'angle2', 'angle3', 'angle4', 'angle5', 'angle6'],
                logger=logger,
                nproc=nproc)
            eval_results['mAP'] = mean_ap
        else:
