...
from mmrotate.core import norm_angle, obb2poly_np, poly2obb_np, obb2xyxy
...
class HRResize(Resize):
    ...
    def _resize_bboxes(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        for key in results.get('bbox_fields_r', []):
            bboxes = results[key]
            orig_shape = bboxes.shape
            bboxes = bboxes.reshape((-1, 5))
            w_scale, h_scale, _, _ = results['scale_factor']
        ...
        for key in results.get('bbox_fields_h', []):
...
class HRRandomFlip(RandomFlip):
    ...
    def __init__(self, flip_ratio=None, direction='horizontal', version='le90'):
        self.version = version
        super(HRRandomFlip, self).__init__(flip_ratio, direction)

    def bbox_flip_r(self, bboxes, img_shape, direction):
        """Flip bboxes horizontally or vertically.
    ...
    def bbox_flip_h(self, bboxes, img_shape, direction):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            # flip h_bboxes
            for key in results.get('bbox_fields_h', []):
                results[key] = self.bbox_flip_h(results[key],
                                              results['img_shape'],
                                              results['flip_direction'])
            # flip r_bboxes
            for key in results.get('bbox_fields_r', []):
                results[key] = self.bbox_flip_r(results[key],
                                              results['img_shape'],
                                              results['flip_direction'])
            # flip masks
...
class ConditionalCopyPaste:
    ...
    def get_indexes(self, dataset):
        ...
        Returns:
            list: Indexes.
        """
        img_id = random.choice(dataset.gun_imgids)
        return dataset.img_ids.index(img_id)
    ...
    def gen_masks_from_bboxes(self, bboxes, img_shape):
        ...
        if bboxes.shape[1] == 5: # rbox
            bboxes = obb2xyxy(torch.from_numpy(bboxes), 'le90').numpy()
    ...
    def get_gt_masks(self, results):
        ...
        if results.get('gt_masks', None) is not None:
            ...
            if 'gt_bboxes' in results.keys():
                ...
                bbox_name = 'gt_bboxes_h'
            return self.gen_masks_from_bboxes(
                    results.get(bbox_name, []), results['img'].shape)
    ...
    def __call__(self, results):
        ...
        if self.selected:
            selected_results = self._select_object(results['mix_results'][0], results['pad_shape'])
        else:
            selected_results = results['mix_results'][0]
        return self._copy_paste(results, selected_results)

    def _select_object(self, results, dst_shape):
        """Select some objects from the source results."""
        # bboxes = results['gt_bboxes']
        if 'gt_bboxes_r' in results.keys():
            bboxes = results['gt_bboxes_h']
            bboxes_r = results['gt_bboxes_r']
        else:
            bboxes_r = results['gt_bboxes']
            bboxes = obb2xyxy(torch.from_numpy(bboxes_r), 'le90').numpy()
        labels = results['gt_labels']
        masks = results['gt_masks']
        # max_num_pasted = min(bboxes.shape[0] + 1, self.max_num_pasted)
        src_h, src_w = results['pad_shape'][0:2]
        dst_h, dst_w = dst_shape[0:2]
        new_masks = np.zeros((bboxes.shape[0], dst_h, dst_w), dtype=np.uint8)
        new_img = np.zeros((dst_h, dst_w, 3), dtype=np.float32)
        
        min_h = min(dst_h, src_h)
        min_w = min(dst_w, src_w)
        for i, mask in enumerate(masks.masks):
            new_masks[i, :min_h, :min_w] = mask[:min_h, :min_w]
        new_masks = BitmapMasks(new_masks, dst_h, dst_w)
        new_img[:min_h, :min_w, :] = results['img'][:min_h, :min_w, :]        
        
        # num_pasted = np.random.randint(0, max_num_pasted)
        
        index = np.where(np.logical_and(results['gt_labels'] == 3, new_masks.areas > 0))[0]
        
        if random.random() < self.paste_rate:
            num_pasted = min(len(index), 1)
        else:
            num_pasted = 0
            
        selected_inds = np.random.choice(
            index, size=num_pasted, replace=False)

        selected_bboxes = bboxes[selected_inds]
        selected_bboxes_r = bboxes_r[selected_inds]
        selected_labels = labels[selected_inds]
        selected_masks = new_masks[selected_inds]


        results['gt_bboxes'] = selected_bboxes_r
        results['gt_bboxes_h'] = selected_bboxes
        results['gt_bboxes_r'] = selected_bboxes_r
    ...
    def _copy_paste(self, dst_results, src_results):
        ...
        # dst_bboxes = dst_results['gt_bboxes_h']
        # dst_bboxes_r = dst_results['gt_bboxes_r']
        dst_labels = dst_results['gt_labels']
        dst_masks = dst_results['gt_masks']
        
        if 'gt_bboxes_r' in dst_results.keys():
            dst_bboxes = dst_results['gt_bboxes_h']
            dst_bboxes_r = dst_results['gt_bboxes_r']
        else:
            dst_bboxes_r = dst_results['gt_bboxes']
            dst_bboxes = obb2xyxy(torch.from_numpy(dst_bboxes_r), 'le90').numpy()

        src_img = src_results['img']
        src_bboxes = src_results['gt_bboxes_h']
        src_bboxes_r = src_results['gt_bboxes_r']
        ...
        bboxes_r = np.concatenate([dst_bboxes_r[valid_inds], src_bboxes_r])
        ...
        img = np.float32(img)
        dst_results['img'] = img
        dst_results['gt_bboxes_h'] = bboxes
        dst_results['gt_bboxes_r'] = bboxes_r
        dst_results['gt_bboxes'] = bboxes_r
